...
void WINAPI resendSendRequire(UINT wTimerID, UINT msg, DWORD dwUser, DWORD dwl, DWORD dw2) {
	...
	sendto(s, "lsend", 5, 0, (SOCKADDR *)&serverListenAddr, serveraddrLen);
	timeOut *= 2;
...
/******************************************上传文件*************************************************/
	if (strncmp(argv[1], "lsend", 5) == 0) {
		...
		sendto(s, "lsend", 5, 0, (SOCKADDR *)&serverListenAddr, serveraddrLen);
		timeOutId = timeSetEvent(timeOut, 1, (LPTIMECALLBACK)resendGetRequire, DWORD(1), TIME_ONESHOT);
		DWORD reqsendtime = GetTickCount();
		...
		if (recvfrom(s, (char *)&serverDataAddr, sizeof(serverDataAddr), 0, NULL, NULL) != -1) {
			...
			estimatedRTT = GetTickCount() - reqsendtime;
			timeOut = 4 * estimatedRTT;
		...
		/*******************开始上传文件************************/
		while (!file.eof()) {
			while (!file.eof() && ((nextseqnum + 1000 - sendbase) % 1000) < (rwnd < cwnd ? rwnd : cwnd)) {
			...
			if (recvfrom(s, (char *)&ackMessage, sizeof(ackMessage), 0, (SOCKADDR *)&rcvDataAddr, &serveraddrLen) != -1) {
				...
				if (ackMessage.ack > packetBuff[sendbase].seq) {
					...
					redundancy = 0;
					sendbase = (sendbase + ackMessage.ack - packetBuff[sendbase].seq) % 1000;
					if (sendbase != nextseqnum) {
						timeOutId = timeSetEvent(timeOut, 1, (LPTIMECALLBACK)resendFileData, DWORD(s), TIME_ONESHOT);
					}
				}
...
/******************************************下载文件*******************************************************/
		...
		cout << "begin downloading file from server" << endl;
		starttime = GetTickCount();
		/*******************开始下载文件*******************/
		...
		while (true) {
			if (recvfrom(s, (char *)&rcvPacket, sizeof(packet), 0, (SOCKADDR *)&recvfromAddr, &serveraddrLen) != -1) {
				...
				//若一分钟未收到数据，则断开连接
				timeOutId = timeSetEvent(60000, 1, (LPTIMECALLBACK)disconnect, DWORD(1), TIME_ONESHOT);
				cout << "-----------------------------------------" << endl;
